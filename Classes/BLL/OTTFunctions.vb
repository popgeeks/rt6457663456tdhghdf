Public Class OTTFunctions
    ...
    Private Sub OTTPlaceCard(ByVal incoming As String, ByVal socket As Integer)
        ...
        Try
            ...
            Dim iTableID As Integer = oTableFunctions.FindPlayersTable(Tables, frmSystray.Winsock(socket).Tag)
            Dim iGameID As Integer = Tables(iTableID).GameID
            ...
            If IsPlayable(iSquareID) = False Then
                ...
                Call blockchat(String.Empty, "Game Ended", String.Concat("Your match was ended because ", frmSystray.Winsock(socket).Tag, " tried to place a card on the OTT board that already had a card on it."), GetSocket(Tables(itableid).player1))
                Call blockchat(String.Empty, "Game Ended", String.Concat("Your match was ended because ", frmSystray.Winsock(socket).Tag, " tried to place a card on the OTT board that already had a card on it."), GetSocket(Tables(itableid).player2))
                objTableFunctions.EndMatch("ENDMATCH " & frmSystray.Winsock(socket).Tag, 0, False, True)
        ...
        Catch ex As Exception
            Call errorsub(ex, "OTTPlaceCard")
            'Call blockchat(String.Empty, "Game Ended", String.Concat("Your match was ended because an error had occured.  A notice has been dispatched to the server operator.  Please try again."), GetSocket(tables(itableid).player1))
            'Call blockchat(String.Empty, "Game Ended", String.Concat("Your match was ended because an error had occured.  A notice has been dispatched to the server operator.  Please try again."), GetSocket(tables(itableid).player2))
    ...
    Private Sub OTTDiscard(ByVal socket As Integer)
        ...
        Try
            Dim iGameID As Integer = Tables(oTableFunctions.FindPlayersTable(Tables, frmSystray.Winsock(socket).Tag)).GameID
    ...
    Public Function OTTChallengeYes(ByVal incoming As String, ByVal socket As Integer, Optional ByVal rulelist As String = "") As String
        ...
        Try
            ...
            End If
        Catch ex As Exception
            Call errorsub(ex, "ottchallengeyes")
    ...
    Private Sub OTTAnalyze(ByVal iSquareID As Integer, ByVal sCardPlayed As String, ByVal iThisPlrID As Integer)
        Try
            Dim oTableFunctions As New BusinessLayer.Tables
            Dim iTableID As Integer = oTableFunctions.FindPlayersTable(Tables, Player1)
            ...
            If IsGameOver() = False Then
                If iThisPlrID = 1 Then
                    Send(String.Empty, "OTTTURNOVER", GetSocket(Tables(iTableID).Player1))
                    Send(String.Empty, "OTTYOURTURN", GetSocket(Tables(iTableID).Player2))
                    Send(String.Empty, String.Concat("OTTCARDDRAW ", Omni_NextCard(2, False), " ", Omni_NextCard(2, True)), GetSocket(Tables(iTableID).Player2))
                Else
                    Send(String.Empty, "OTTTURNOVER", GetSocket(Tables(iTableID).Player2))
                    Send(String.Empty, "OTTYOURTURN", GetSocket(Tables(iTableID).Player1))
                    Send(String.Empty, String.Concat("OTTCARDDRAW ", Omni_NextCard(1, False), " ", Omni_NextCard(1, True)), GetSocket(Tables(iTableID).Player1))
                End If
            Else
                GameOver(0, False)
            End If
        Catch ex As Exception
            Call errorsub(ex, "OTTAnalyze")
        End Try
    End Sub
    ...
    Public Sub GameOver(ByVal iSkip As Integer, ByVal blnTimeout As Boolean)
        Try
            ...
            If Player1_Score > Player2_Score Then
                'Player 1 won
                oGameDAL.RankChange(Player1, Player2, "OTT")
            ...
            ElseIf Player1_Score < Player2_Score Then
                ...
                oGameDAL.RankChange(Player2, Player1, "OTT")
            ...
            Send(String.Empty, "OTTCLOSE", GetSocket(Tables(itableid).player1))
            Send(String.Empty, "OTTCLOSE", GetSocket(Tables(itableid).player2))

            Dim bTurbo As Boolean = False

            'If Me.GameLength >= oServerConfig.TurboMinimum And Player1_Score <> Player2_Score Then
            Call gpadd(sWinner, iWinner_Gold, True)
            Call gpadd(sLoser, iLoser_Gold, True)

            Call oGameFunctions.GiveExp(sWinner, IIf(Me.Winner = 1, Player1_Score, Player2_Score), sLoser, IIf(Me.Winner = 1, Player1_Score, Player2_Score), GameID, Nothing, True, "ott")
            'ElseIf Player1_Score <> Player2_Score Then
            ...
            If blnSkip = False And bTurbo = False Then oGameFunctions.EndGameEvent(Tables(iTableID))

            EndGame()
        Catch ex As Exception
            Call errorsub(ex, "OTT.GameOver")
        Finally
            Dim oTableFunctions As New BusinessLayer.Tables
    ...
    Private Sub OTTMorePower(ByVal iSquareID As Integer, ByVal iThisPlrID As Integer, ByVal sCardPlayed As String, ByVal sPlayedSpot_Direction As String, ByVal iNeighborID As Integer, ByVal sNeighbor_Direction As String)
        Dim sNeighborCard As String = String.Empty
        ...
        Try
            ...
            If ignoremove = False Then
                ...
                If oPlacedCard.DirectionValue(sPlayedSpot_Direction) > oNeighborCard.DirectionValue(sNeighbor_Direction) Then
                    ...
                    sNeighborCard = oNeighborCard.CardName ' Debug
            ...
            End If
        Catch ex As Exception
            Call errorsub(String.Concat(ex.ToString, vbCrLf, vbCrLf, "Card/SquareID: ", sCardPlayed, "/", iSquareID, " - Neighbor/NeighborID: ", sNeighborCard, "/", iNeighborID, " - GameID:" & GameID), "ottMorePower")
        End Try
    End Sub
    ...
    Private Sub FindOttSendPoints()
        Try
            ...
            Send(String.Empty, String.Concat("OTTPOINTS ", Player1_Score, " ", Player2_Score), GetSocket(Tables(itableid).player1))
            Send(String.Empty, String.Concat("OTTPOINTS ", Player1_Score, " ", Player2_Score), GetSocket(Tables(itableid).player2))
        Catch ex As Exception
            Call errorsub(ex, "FindOttSendPoints")
        End Try
    End Sub
    ...
    Private Sub SendOTTCardFlip(ByVal iBoardID As Integer, ByVal iNeighborID As Integer)
        ...
        Send(String.Empty, String.Concat("OTTCARDFLIP ", iNeighborID, " ", status, " ", CardColor(iNeighborID)), GetSocket(Tables(itableid).player1))
        Send(String.Empty, String.Concat("OTTCARDFLIP ", iNeighborID, " ", status, " ", CardColor(iNeighborID)), GetSocket(Tables(itableid).player2))
    End Sub
    ...
    Private Sub SendOTTCardData(ByVal iBoardID As Integer)
        ...
        Select Case CardColor(iBoardID)
            Case "blue"
                Send(String.Empty, String.Concat("OTTBLUECARD ", iBoardID, " ", Board(iBoardID).Replace(" ", "%20")), GetSocket(Tables(itableid).player1))
                Send(String.Empty, String.Concat("OTTBLUECARD ", iBoardID, " ", Board(iBoardID).Replace(" ", "%20")), GetSocket(Tables(itableid).player2))
            Case "red"
                Send(String.Empty, String.Concat("OTTREDCARD ", iBoardID, " ", Board(iBoardID).Replace(" ", "%20")), GetSocket(Tables(itableid).player1))
                Send(String.Empty, String.Concat("OTTREDCARD ", iBoardID, " ", Board(iBoardID).Replace(" ", "%20")), GetSocket(Tables(itableid).player2))
        End Select
    End Sub
    ...
    Private Sub OTTSameData(ByVal pointchange As Integer)
        Try
            ...
            If pointchange > 0 Then
                Send(String.Empty, "OTTSAME", GetSocket(Tables(itableid).player1))
                Send(String.Empty, "OTTSAME", GetSocket(Tables(itableid).player2))

                HistoryInsert(-1, String.Empty, "SAME")
            End If
        Catch ex As Exception
            Call errorsub(ex, "ottsamedata")
        End Try
    End Sub
    ...
    Private Sub OTTPlusData(ByVal pointchange As Integer)
        Try
            ...
            If pointchange > 0 Then
                Send(String.Empty, "OTTPLUS", GetSocket(Tables(itableid).player1))
                Send(String.Empty, "OTTPLUS", GetSocket(Tables(itableid).player2))

                HistoryInsert(-1, String.Empty, "PLUS")
            End If
        Catch ex As Exception
            Call errorsub(ex, "ottplusdata")
        End Try
    End Sub
    ...
    Private Sub OTTComboData(ByVal pointchange As Integer)
        Try
            ...
            If pointchange > 0 Then
                Send(String.Empty, "OTTCOMBO", GetSocket(Tables(itableid).player1))
                Send(String.Empty, "OTTCOMBO", GetSocket(Tables(itableid).player2))

                HistoryInsert(-1, String.Empty, "COMBO")
            End If
        Catch ex As Exception
            Call errorsub(ex, "OTTComboData")
        End Try
    End Sub
End Class
