...
skip:
    ...
    Private Sub team_gamechat(ByVal incoming As String, ByVal socket As Integer)
        Try
            ...
            Dim itableID As Integer, ThisPlrId As Integer

            strChat = Mid(incoming, 15, Len(incoming))
            itableID = oTables.FindPlayersTable(Tables, frmSystray.Winsock(socket).Tag)

            If itableID = 0 Then Exit Sub
            Call DalNew(Tables(itableID).GameID)
            ...
            If Left(strChat, 5) <> "/team" Then
                If ThisPlrId <> 1 Then Send(String.Empty, "TEAM_GAMECHAT " & frmSystray.Winsock(socket).Tag & " " & strChat, GetSocket(Tables(itableID).Player1))
                If ThisPlrId <> 2 Then Send(String.Empty, "TEAM_GAMECHAT " & frmSystray.Winsock(socket).Tag & " " & strChat, GetSocket(Tables(itableID).Player2))
                If ThisPlrId <> 3 Then Send(String.Empty, "TEAM_GAMECHAT " & frmSystray.Winsock(socket).Tag & " " & strChat, GetSocket(Tables(itableID).Player3))
                If ThisPlrId <> 4 Then Send(String.Empty, "TEAM_GAMECHAT " & frmSystray.Winsock(socket).Tag & " " & strChat, GetSocket(Tables(itableID).Player4))
            Else
                Select Case ThisPlrId
                    Case 1
                        Send(String.Empty, "TEAM_GAMECHAT " & frmSystray.Winsock(socket).Tag & " " & strChat, GetSocket(Tables(itableID).Player2))
                    Case 2
                        Send(String.Empty, "TEAM_GAMECHAT " & frmSystray.Winsock(socket).Tag & " " & strChat, GetSocket(Tables(itableID).Player1))
                    Case 3
                        Send(String.Empty, "TEAM_GAMECHAT " & frmSystray.Winsock(socket).Tag & " " & strChat, GetSocket(Tables(itableID).Player4))
                    Case 4
                        Send(String.Empty, "TEAM_GAMECHAT " & frmSystray.Winsock(socket).Tag & " " & strChat, GetSocket(Tables(itableID).Player3))
                End Select
            End If
        Catch ioorex As System.IndexOutOfRangeException
            ' Do nothing
        Catch ex As Exception
            Call errorsub(ex, "team_gamechat")
        End Try
    End Sub
    ...
    Private Function teamomnichallengeyes(ByVal tableID As Integer) As String
        Try
            ...
            Return GameID
        Catch ex As Exception
            Call errorsub(ex, "teamomnichallengeyes")
            Return String.Empty
        End Try
    End Function
            ...
            If IsGameOver() = False Then
                Select Case iThisPlrID
                    Case 1
                        ...
                        Send(String.Empty, "TEAM_OTTTURNOVER " & Player3, GetSocket(Tables(itableid).player1))
                        Send(String.Empty, "TEAM_OTTTURNOVER " & Player3, GetSocket(Tables(itableid).player2))
                        Send(String.Empty, "TEAM_OTTYOURTURN", GetSocket(Tables(itableid).player3))
                        Send(String.Empty, "TEAM_OTTTURNOVER " & Player3, GetSocket(Tables(itableid).player4))
                        Send(Player3, "TEAM_OTTCARDDRAW " & Omni_NextCard(3, False) & " " & Omni_NextCard(3, True))
                    Case 2
                        ...
                        Send(String.Empty, "TEAM_OTTTURNOVER " & Player3, GetSocket(Tables(itableid).player1))
                        Send(String.Empty, "TEAM_OTTTURNOVER " & Player3, GetSocket(Tables(itableid).player2))
                        Send(String.Empty, "TEAM_OTTYOURTURN", GetSocket(Tables(itableid).player4))
                        Send(String.Empty, "TEAM_OTTTURNOVER " & Player3, GetSocket(Tables(itableid).player3))
                        Send(Player4, "TEAM_OTTCARDDRAW " & Omni_NextCard(4, False) & " " & Omni_NextCard(4, True))
                    Case 3
                        ...
                        Send(String.Empty, "TEAM_OTTTURNOVER " & Player3, GetSocket(Tables(itableid).player1))
                        Send(String.Empty, "TEAM_OTTTURNOVER " & Player3, GetSocket(Tables(itableid).player3))
                        Send(String.Empty, "TEAM_OTTYOURTURN", GetSocket(Tables(itableid).player2))
                        Send(String.Empty, "TEAM_OTTTURNOVER " & Player3, GetSocket(Tables(itableid).player4))
                        Send(Player2, "TEAM_OTTCARDDRAW " & Omni_NextCard(2, False) & " " & Omni_NextCard(2, True))
                    Case 4
                        ...
                        Send(String.Empty, "TEAM_OTTTURNOVER " & Player3, GetSocket(Tables(itableid).player2))
                        Send(String.Empty, "TEAM_OTTTURNOVER " & Player3, GetSocket(Tables(itableid).player3))
                        Send(String.Empty, "TEAM_OTTYOURTURN", GetSocket(Tables(itableid).player1))
                        Send(String.Empty, "TEAM_OTTTURNOVER " & Player3, GetSocket(Tables(itableid).player4))
                        Send(Player1, "TEAM_OTTCARDDRAW " & Omni_NextCard(1, False) & " " & Omni_NextCard(1, True))
                End Select
            Else
    ...
    Private Sub TeamGameOver()
        Try
            ...
            Send(String.Empty, "TEAM_OTTTURNOVER", GetSocket(Tables(itableid).player1))
            Send(String.Empty, "TEAM_OTTTURNOVER", GetSocket(Tables(itableid).player2))
            Send(String.Empty, "TEAM_OTTTURNOVER", GetSocket(Tables(itableid).player3))
            Send(String.Empty, "TEAM_OTTTURNOVER", GetSocket(Tables(itableid).player4))

            Send(String.Empty, "TEAM_OTTCLOSE", GetSocket(Tables(itableid).player1))
            Send(String.Empty, "TEAM_OTTCLOSE", GetSocket(Tables(itableid).player2))
            Send(String.Empty, "TEAM_OTTCLOSE", GetSocket(Tables(itableid).player3))
            Send(String.Empty, "TEAM_OTTCLOSE", GetSocket(Tables(itableid).player4))

            EndGame()

            Dim oGameFunctions As New GameFunctions
            oGameFunctions.EndGameEvent(Tables(iTableID))
        Catch ex As Exception
            Call errorsub(ex, "TeamGameOver")
        Finally
            Dim oTables As New BusinessLayer.Tables
    ...
    Private Sub team_ottMorePower(ByVal iSquareID As Integer, ByVal iThisPlrID As Integer, ByVal sCardPlayed As String, ByVal sPlayedSpot_Direction As String, ByVal iNeighborID As Integer, ByVal sNeighbor_Direction As String)
        Try
            ...
            End If
        Catch ex As Exception
            Call errorsub(ex, "team_ottMorePower")
        End Try
    End Sub
    ...
    Private Sub team_FindOttSendPoints()
        Try
            ...
            Send(String.Empty, String.Concat("TEAM_OTTPOINTS ", Team1_Score, " ", Team2_Score), GetSocket(Tables(itableid).player1))
            Send(String.Empty, String.Concat("TEAM_OTTPOINTS ", Team1_Score, " ", Team2_Score), GetSocket(Tables(itableid).player2))
            Send(String.Empty, String.Concat("TEAM_OTTPOINTS ", Team1_Score, " ", Team2_Score), GetSocket(Tables(itableid).player3))
            Send(String.Empty, String.Concat("TEAM_OTTPOINTS ", Team1_Score, " ", Team2_Score), GetSocket(Tables(itableid).player4))
        Catch ex As Exception
            Call errorsub(ex, "team_FindOttSendPoints")
        End Try
    End Sub
    ...
    Private Sub sendteam_ottcardflip(ByVal iBoardID As Integer, ByVal iNeighborID As Integer)
        ...
        Send(String.Empty, String.Concat("TEAM_OTTCARDFLIP ", iNeighborID, " ", status, " ", CardColor(iNeighborID)), GetSocket(Tables(itableid).player1))
        Send(String.Empty, String.Concat("TEAM_OTTCARDFLIP ", iNeighborID, " ", status, " ", CardColor(iNeighborID)), GetSocket(Tables(itableid).player2))
        Send(String.Empty, String.Concat("TEAM_OTTCARDFLIP ", iNeighborID, " ", status, " ", CardColor(iNeighborID)), GetSocket(Tables(itableid).player3))
        Send(String.Empty, String.Concat("TEAM_OTTCARDFLIP ", iNeighborID, " ", status, " ", CardColor(iNeighborID)), GetSocket(Tables(itableid).player4))
    End Sub
    ...
    Private Sub team_ottchecksame(ByVal sCardPlayed As String, ByVal iBoardID As Integer, ByVal sPlayedSpot_Direction1 As String, ByVal sPlayedSpot_Direction2 As String, ByVal iNeighborID As Integer, ByVal sNeighbor_Direction1 As String, ByVal iNeighbor2ID As Integer, ByVal sNeighbor_Direction2 As String, ByVal iThisPlrID As Integer)
        Try
            ...
            End If
        Catch ex As Exception
            Call errorsub(ex, "team_ottchecksame")
        End Try
    End Sub
    ...
    Public Sub team_ottsamedata(ByVal pointchange As Integer)
        Try
            ...
            If pointchange > 0 Then
                Send(String.Empty, "TEAM_OTTSAME", GetSocket(Tables(itableid).player1))
                Send(String.Empty, "TEAM_OTTSAME", GetSocket(Tables(itableid).player2))
                Send(String.Empty, "TEAM_OTTSAME", GetSocket(Tables(itableid).player3))
                Send(String.Empty, "TEAM_OTTSAME", GetSocket(Tables(itableid).player4))

                HistoryInsert(-1, String.Empty, "SAME")
            End If
        Catch ex As Exception
            Call errorsub(ex, "team_ottsamedata")
        End Try
    End Sub
    ...
    Public Sub team_ottplusdata(ByVal pointchange As Integer)
        Try
            ...
            If pointchange > 0 Then
                Send(String.Empty, "TEAM_OTTPLUS", GetSocket(Tables(itableid).player1))
                Send(String.Empty, "TEAM_OTTPLUS", GetSocket(Tables(itableid).player2))
                Send(String.Empty, "TEAM_OTTPLUS", GetSocket(Tables(itableid).player3))
                Send(String.Empty, "TEAM_OTTPLUS", GetSocket(Tables(itableid).player4))

                HistoryInsert(-1, String.Empty, "PLUS")
            End If
        Catch ex As Exception
            Call errorsub(ex, "team_ottplusdata")
        End Try
    End Sub
    ...
    Private Sub team_ottcheckplus(ByVal sCardPlayed As String, ByVal iBoardID As Integer, ByVal sPlayedSpot_Direction1 As String, ByVal sPlayedSpot_Direction2 As String, ByVal iNeighborID As Integer, ByVal sNeighbor_Direction1 As String, ByVal iNeighbor2ID As Integer, ByVal sNeighbor_Direction2 As String, ByVal iThisPlrID As Integer)
        Try
            ...
            End If
        Catch ex As Exception
            Call errorsub(ex, "Team_OTTCheckPlus")
        End Try
    End Sub
    ...
    Public Sub team_ottcombosub(ByVal sPlayedCard As String, ByVal iBoardID As Integer, ByVal iThisPlrID As Integer)
        Try
            ...
            End Select
        Catch ex As Exception
            Call errorsub(ex, "team_OTTComboSub")
        End Try
    End Sub
    ...
    Public Sub team_ottcheckcombo(ByVal sCardPlayed As String, ByVal iThisPlrID As Integer, ByVal iBoardID As Integer, ByVal sPlayedSpot_Direction1 As String, ByVal iNeighborID As Integer, ByVal sNeighbor_Direction1 As String)
        Try
            ...
            End If
        Catch ex As Exception
            Call errorsub(ex, "OTTCheckCombo")
        End Try
    End Sub
    ...
    Public Sub team_ottcombodata(ByVal pointchange As Integer)
        Try
            ...
            If pointchange > 0 Then
                Send(String.Empty, "TEAM_OTTCOMBO", GetSocket(Tables(itableid).player1))
                Send(String.Empty, "TEAM_OTTCOMBO", GetSocket(Tables(itableid).player2))
                Send(String.Empty, "TEAM_OTTCOMBO", GetSocket(Tables(itableid).player3))
                Send(String.Empty, "TEAM_OTTCOMBO", GetSocket(Tables(itableid).player4))

                HistoryInsert(-1, String.Empty, "COMBO")
            End If
        Catch ex As Exception
            Call errorsub(ex, "team_OTTComboData")
        End Try
    End Sub
    ...
    Public Sub sendteam_ottcarddata(ByVal iBoardID As Integer)
        ...
        Send(String.Empty, String.Concat("TEAM_OTT", UCase(CardColor(iBoardID)), "CARD ", iBoardID, " ", Board(iBoardID).Replace(" ", "%20")), GetSocket(Tables(itableid).player1))
        Send(String.Empty, String.Concat("TEAM_OTT", UCase(CardColor(iBoardID)), "CARD ", iBoardID, " ", Board(iBoardID).Replace(" ", "%20")), GetSocket(Tables(itableid).player2))
        Send(String.Empty, String.Concat("TEAM_OTT", UCase(CardColor(iBoardID)), "CARD ", iBoardID, " ", Board(iBoardID).Replace(" ", "%20")), GetSocket(Tables(itableid).player3))
        Send(String.Empty, String.Concat("TEAM_OTT", UCase(CardColor(iBoardID)), "CARD ", iBoardID, " ", Board(iBoardID).Replace(" ", "%20")), GetSocket(Tables(itableid).player4))
    End Sub
    ...
    Private Sub teamgiveexp()
        ...
        With Me
            ...
            For x As Integer = 1 To 2
                ...
                oGameFunctions.RushBoost(.PlayerNick(1))
                oGameFunctions.RushBoost(.PlayerNick(2))
                oGameFunctions.RushBoost(.PlayerNick(3))
                oGameFunctions.RushBoost(.PlayerNick(4))
            Next x
        End With
    End Sub
    ...
    Public Sub teamladdersub()
        Try
            ...
            End With
        Catch ex As Exception
            Call errorsub(ex, "teamladdersub")
        End Try
    End Sub
