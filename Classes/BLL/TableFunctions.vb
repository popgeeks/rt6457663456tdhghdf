Public Class TableFunctions
    Public Sub IncomingModule(ByVal sCommand As String, ByVal incoming As String, ByVal socket As Integer)
        ...
        Select Case sCommand
            ...
            Case "TABLEJACKPOTCHANGE"
                Call tablejackpotchange(incoming, socket)
            Case "TABLEBOT"
                Call tablebot(incoming, socket)
        End Select
    End Sub
    ...
    Private Sub jointeamtable(ByVal incoming As String, ByVal socket As Integer)
        ...
        Catch ex As Exception
            Call errorsub(ex, "jointeamtable")
            Call blockchat(String.Empty, "Error", "An error occured.  You cannot join this table.  Please try again later.", socket)
        End Try
    End Sub
    ...
    Private Sub tablejackpotchange(ByVal incoming As String, ByVal socket As Integer)
        Dim sItems() As String = incoming.Split()
        Dim sWindow As String = sItems(1)

        Dim oTableFunctions As New BusinessLayer.Tables
        Dim iTableID As Integer = oTableFunctions.FindPlayersTable(Tables, players(socket).nickname)

        If Tables(iTableID).GameID > 0 Then
            Send(String.Empty, "TABLEJACKPOTCHANGE 2 " & sWindow, socket)
        ElseIf players(socket).nickname = Tables(iTableID).Player1 Then
            If Tables(iTableID).Player1_Jackpot = True Then
                Tables(iTableID).Player1_Jackpot = False
            Else
                Tables(iTableID).Player1_Jackpot = True
            End If
        ElseIf players(socket).nickname = Tables(iTableID).Player2 Then
            If Tables(iTableID).Player2_Jackpot = True Then
                Tables(iTableID).Player2_Jackpot = False
            Else
                Tables(iTableID).Player2_Jackpot = True
            End If
        ElseIf players(socket).nickname = Tables(iTableID).Player3 Then
            If Tables(iTableID).Player3_Jackpot = True Then
                Tables(iTableID).Player3_Jackpot = False
            Else
                Tables(iTableID).Player3_Jackpot = True
            End If
        ElseIf players(socket).nickname = Tables(iTableID).Player4 Then
            If Tables(iTableID).Player4_Jackpot = True Then
                Tables(iTableID).Player4_Jackpot = False
            Else
                Tables(iTableID).Player4_Jackpot = True
            End If
        Else
            Send(String.Empty, "TABLEJACKPOTCHANGE 0 " & sWindow, socket)
            Exit Sub
        End If

        Send(String.Empty, "TABLEJACKPOTCHANGE 1 " & sWindow, socket)
    End Sub

    Private Sub tablebot(ByVal incoming As String, ByVal socket As Integer)
        Dim sGame As String = String.Empty, sBot As String = String.Empty
        Dim lostvar As String = String.Empty

        Divide(incoming, " ", lostvar, sGame, sBot)

        If sBot = String.Empty Then

        ElseIf sGame = String.Empty Then
