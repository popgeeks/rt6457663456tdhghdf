...
Public Class AdminFunctions
    ...
    Private Sub kick(ByVal incoming As String, ByVal socket As Integer)
        Try
            ...
            If (oAdminAccount.AdminLevel <> 1) And (oPlayerAccount.AdminLevel <> 4) And (oAdminAccount.AdminLevel <> 0) And socket <> 0 Then
                ...
                'oChatFunctions.unisend("ADMINOPCOMMAND", String.Concat("kick ", player, " ", oAdminAccount.Player, " ", reason.Replace(" ", "%20")))

                Call quit("QUIT " & player, oPlayerAccount.PlayerSocket, False)
                VB6.SetItemString(frmserver.lstplayers, oPlayerAccount.PlayerSocket, oPlayerAccount.PlayerSocket & ":")
            End If
        Catch ex As Exception
            Call errorsub(ex.ToString, "kick")
        End Try
    End Sub
    ...
    Private Sub warn(ByVal incoming As String, ByVal socket As Integer)
        Try
            ...
            End If
        Catch ex As Exception
            Call errorsub(ex.ToString, "warn")
        End Try
    End Sub
    ...
    Private Sub adminmod(ByVal incoming As String, ByVal socket As Integer)
        Try
            ...
            End If
        Catch ex As Exception
            Call errorsub(ex.ToString, "adminmod")
        End Try
    End Sub
    ...
    Private Sub adminchat(ByVal incoming As String, ByVal socket As Integer)
        ...
        If (oPlayerAccount.AdminLevel <> 0) And socket <> 0 Then
            If frmserver.lstadmin.Items.Count = 0 Then
            ...
            For x As Integer = 0 To frmserver.lstadmin.Items.Count - 1
                ...
                Divide(frmserver.lstadmin.Items.Item(x).ToString, " ", strAdmin)
    ...
    Private Sub adminlogin(ByVal incoming As String, ByVal socket As Integer)
        Try
            ...
            If (oPlayerAccount.AdminLevel = 4) And LCase(frmSystray.Winsock(socket).Tag) <> "darklumina" And LCase(frmSystray.Winsock(socket).Tag) <> "atomsplit" Then
                Send(String.Empty, "ADMINSTATUS 4", socket)
                RemAdmin(frmSystray.Winsock(socket).Tag)
                frmserver.lstadmin.Items.Add(frmSystray.Winsock(socket).Tag & " 4 *")
            ElseIf (oPlayerAccount.AdminLevel = 3) Then
                Send(String.Empty, "ADMINSTATUS 3", socket)
                RemAdmin(frmSystray.Winsock(socket).Tag)
                frmserver.lstadmin.Items.Add(frmSystray.Winsock(socket).Tag & " 3 @")
            ElseIf (oPlayerAccount.AdminLevel = 2) Then
                Send(String.Empty, "ADMINSTATUS 2", socket)
                RemAdmin(frmSystray.Winsock(socket).Tag)
                frmserver.lstadmin.Items.Add(frmSystray.Winsock(socket).Tag & " 2 @")
            ElseIf (oPlayerAccount.AdminLevel = 1) Then
                Send(String.Empty, "ADMINSTATUS 1", socket)
                RemAdmin(frmSystray.Winsock(socket).Tag)
                frmserver.lstadmin.Items.Add(frmSystray.Winsock(socket).Tag & " 1 +")
            ElseIf (oPlayerAccount.AdminLevel = 4) And (LCase(frmSystray.Winsock(socket).Tag) = "darklumina") Or (LCase(frmSystray.Winsock(socket).Tag) = "atomsplit") Then
                Send(String.Empty, "ADMINSTATUS 5", socket)
                RemAdmin(frmSystray.Winsock(socket).Tag)
                frmserver.lstadmin.Items.Add(frmSystray.Winsock(socket).Tag & " 5 *")
            Else
        ...
        Catch ex As Exception
            Call errorsub(ex.ToString, "adminlogin")
        End Try
    End Sub
    ...
    Private Sub adminsilenceplayer(ByVal incoming As String, ByVal socket As Integer)
        Try
            ...
            If oPlayerAccount.AdminLevel <> 0 Then
                ...
                Call blockchat(sNick, "Silence", "You have been silenced by " & frmSystray.Winsock(socket).Tag & ".  You may not talk for 5 minutes.")
                ...
                Call oChatFunctions.unisend("CHATBLOCK", "Silence " & String.Concat(sNick, " has been silenced for 5 minutes by ", frmSystray.Winsock(socket).Tag, ".").Replace(" ", "%20"))
            End If
        Catch ex As Exception
            Call errorsub(ex.ToString, "adminsilenceplayer")
        End Try
    End Sub
    ...
    Private Sub adminkeyword(ByVal incoming As String, ByVal socket As Integer)
        ...
        Catch ex As Exception
            Call errorsub(ex.ToString, "adminkeyword")
        End Try
    End Sub
    ...
    Private Sub adminremovetrigger(ByVal incoming As String, ByVal socket As Integer)
        Try
            ...
            Next x
        Catch ex As Exception
            Call errorsub(ex.ToString, "adminremovetrigger")
        End Try
    End Sub
End Class
