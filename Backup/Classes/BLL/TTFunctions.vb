Public Class TTFunctions
            ...
            Select Case ThisPlrID
                Case 2
                    For x As Integer = 1 To CardstoTake
                        ...
                        Call blockchat(String.Empty, "Cards Won", String.Concat("You gained 1 ", sCard), PlayerSocket(1))
                ...
                Case 1
                    For x As Integer = 1 To CardstoTake
                        ...
                        Call blockchat(String.Empty, "Cards Won", String.Concat("You gained 1 ", sCard), PlayerSocket(2))
        ...
        Try
            ...
            Select Case Winner
                Case 1
                    ...
                    Call blockchat(String.Empty, "Cards Won", String.Concat("You gained 1 ", sCard), PlayerSocket(1))
                    Call blockchat(String.Empty, "Cards Lost", String.Concat("You lost 1 ", sCard), PlayerSocket(2))
                Case 2
                    Randomize()
                    Dim iRandomNumber As Integer = Int(Rnd() * 5)
                    ...
                    Call blockchat(String.Empty, "Cards Won", String.Concat("You gained 1 ", sCard), PlayerSocket(2))
                    Call blockchat(String.Empty, "Cards Lost", String.Concat("You lost 1 ", sCard), PlayerSocket(1))
                Case 0
                    ' Do nothing, warn admins
                    Call blockchat(String.Empty, "Error", "You cannot pick a card because you have already received cards for this game or you are not awarded cards.", socket)
            ...
            Update()
        Catch ex As Exception
            Call errorsub(ex.ToString, "tt_takerandomcard")
        End Try
    End Sub
    ...
    Private Sub tt_takecard(ByVal incoming As String, ByVal socket As Integer)
        Try
            ...
            Send(String.Empty, "TT_CARDGAIN " & sItems(2) & " " & sItems(3) & " " & sItems(4) & " " & sItems(5) & " " & sItems(6), IIf(ThisPlrID = 1, PlayerSocket(1), PlayerSocket(2)))

            'For t As Integer = 2 To 6
            'If sItems(t) <> String.Empty Then Send(String.Empty, "CHATBLOCK Received You%20received%201%20" & sItems(t), IIf(ThisPlrID = 1, PlayerSocket(1), PlayerSocket(2)))
            'Next t

            Send(String.Empty, "TT_CARDLOST " & sItems(2) & " " & sItems(3) & " " & sItems(4) & " " & sItems(5) & " " & sItems(6), IIf(ThisPlrID = 1, PlayerSocket(2), PlayerSocket(1)))

            '            For t As Integer = 2 To 6
            ' If sItems(t) <> String.Empty Then Send(String.Empty, "CHATBLOCK Lost You%20lost%201%20" & sItems(t), IIf(ThisPlrID = 1, PlayerSocket(2), PlayerSocket(1)))
            'Next t

            CardsTaken = True
            Update()
        Catch ex As Exception
            Call errorsub(ex.ToString, "tt_takecard")
        End Try
    End Sub
    ...
    Private Sub tt_timeout(ByVal socket As Integer)
        Try
            ...
            If PlayerSocket(1) <> 0 Then Call blockchat(String.Empty, "Cancelled", "Your challenge was cancelled by the server because your opponent timed out.  Please try again later.", PlayerSocket(1))
            If PlayerSocket(2) <> 0 Then Call blockchat(String.Empty, "Cancelled", "Your challenge was cancelled by the server because your opponent timed out.  Please try again later.", PlayerSocket(2))
            ...
            EndGame()
        Catch ex As Exception
            Call errorsub(ex.ToString, "tt_timeout")
        End Try
    End Sub
    ...
    Private Sub mycard(ByVal incoming As String, ByVal socket As Integer)
        Try
            ...
            For x As Integer = 0 To 4
                ...
                Else
                    ...
                    Else
                        ...
                        Call blockchat(String.Empty, "Game Ended", String.Concat("Your match was ended because ", frmSystray.Winsock(socket).Tag, " tried to pick a special card in a trade game."), PlayerSocket(1))
                        Call blockchat(String.Empty, "Game Ended", String.Concat("Your match was ended because ", frmSystray.Winsock(socket).Tag, " tried to pick a special card in a trade game."), PlayerSocket(2))
            ...
            End If
        Catch ex As Exception
            Call errorsub(ex.ToString, "mycard")
        End Try
    End Sub
    ...
    Private Sub SendOpenCards()
        ...
        Player_Hand(2, 4).Replace(" ", "%20")), PlayerSocket(1))
        ...
        Player_Hand(1, 4).Replace(" ", "%20")), PlayerSocket(2))
    End Sub
    ...
    Private Sub ready()
        Try
            ...
            If ValidateCards() = True Then
                Send(String.Empty, String.Concat("TT_STARTER ", iGameStarter), PlayerSocket(1))
                Send(String.Empty, String.Concat("TT_STARTER ", iGameStarter), PlayerSocket(2))
            End If
        Catch ex As Exception
            Call errorsub(ex.ToString, "ready")
        End Try
    End Sub
    ...
    Private Function ValidateCards() As Boolean
        Try
            ...
            Return True
        Catch ex As Exception
            Call errorsub(ex.ToString, "ValidateCards")
        End Try
    End Function
    ...
    Public Function challengeyes(ByVal incoming As String, ByVal socket As Integer, Optional ByVal rulelist As String = "", Optional ByVal decklist As String = "") As String
        ...
        PlayerSocket(1) = GetSocket(Player1)
        ...
        Send(String.Empty, String.Concat("CHALLENGEYES ", frmSystray.Winsock(socket).Tag), PlayerSocket(1))
        ...
        If Random = True And FullDeck = False Then
            If RandomCards(Player1, 1) = True And RandomCards(Player2, 2) = True Then
                Send(String.Empty, "TT_PICKCARDS 0", PlayerSocket(1))
                Send(String.Empty, "TT_PICKCARDS 0", PlayerSocket(2))

                SendCards()
            Else
                Dim oTableFunctions As New BusinessLayer.Tables
                Call blockchat(String.Empty, "Cancelled", "Your challenge was cancelled by the server. One of the players may not have had enough cards to play with the decks that were chosen. Please try again later.", PlayerSocket(1))
                Call blockchat(String.Empty, "Cancelled", "Your challenge was cancelled by the server.  One of the players may not have had enough cards to play with the decks that were chosen. Please try again later.", PlayerSocket(2))
        ...
        ElseIf FullDeck = True Then
            ...
            End If
        Else
            Send(String.Empty, "TT_PICKCARDS", PlayerSocket(1))
            Send(String.Empty, "TT_PICKCARDS", PlayerSocket(2))
        End If
        ...
        If Elemental = True Then
            ...
            If q <> 0 Then
                For r As Integer = 1 To q
                    ...
                    If enumber <= 10 Then
                        Element(i) = "thunder"
                        Send(String.Empty, String.Concat("TT_ELEMENT ", i, " thunder"), PlayerSocket(1))
                        Send(String.Empty, String.Concat("TT_ELEMENT ", i, " thunder"), PlayerSocket(2))
                    ElseIf enumber <= 20 Then
                        Element(i) = "earth"
                        Send(String.Empty, String.Concat("TT_ELEMENT ", i, " earth"), PlayerSocket(1))
                        Send(String.Empty, String.Concat("TT_ELEMENT ", i, " earth"), PlayerSocket(2))
                    ElseIf enumber <= 30 Then
                        Element(i) = "ice"
                        Send(String.Empty, String.Concat("TT_ELEMENT ", i, " ice"), PlayerSocket(1))
                        Send(String.Empty, String.Concat("TT_ELEMENT ", i, " ice"), PlayerSocket(2))
                    ElseIf enumber <= 40 Then
                        Element(i) = "wind"
                        Send(String.Empty, String.Concat("TT_ELEMENT ", i, " wind"), PlayerSocket(1))
                        Send(String.Empty, String.Concat("TT_ELEMENT ", i, " wind"), PlayerSocket(2))
                    ElseIf enumber <= 50 Then
                        Element(i) = "poison"
                        Send(String.Empty, String.Concat("TT_ELEMENT ", i, " poison"), PlayerSocket(1))
                        Send(String.Empty, String.Concat("TT_ELEMENT ", i, " poison"), PlayerSocket(2))
                    ElseIf enumber <= 60 Then
                        Element(i) = "holy"
                        Send(String.Empty, String.Concat("TT_ELEMENT ", i, " holy"), PlayerSocket(1))
                        Send(String.Empty, String.Concat("TT_ELEMENT ", i, " holy"), PlayerSocket(2))
                    ElseIf enumber <= 70 Then
                        Element(i) = "fire"
                        Send(String.Empty, String.Concat("TT_ELEMENT ", i, " fire"), PlayerSocket(1))
                        Send(String.Empty, String.Concat("TT_ELEMENT ", i, " fire"), PlayerSocket(2))
                    ElseIf enumber <= 80 Then
                        Element(i) = "water"
                        Send(String.Empty, String.Concat("TT_ELEMENT ", i, " water"), PlayerSocket(1))
                        Send(String.Empty, String.Concat("TT_ELEMENT ", i, " water"), PlayerSocket(2))
                    ElseIf enumber <= 90 Then
                        Element(i) = "mechanical"
                        Send(String.Empty, String.Concat("TT_ELEMENT ", i, " mechanical"), PlayerSocket(1))
                        Send(String.Empty, String.Concat("TT_ELEMENT ", i, " mechanical"), PlayerSocket(2))
                    Else
                        Element(i) = "shadow"
                        Send(String.Empty, String.Concat("TT_ELEMENT ", i, " shadow"), PlayerSocket(1))
                        Send(String.Empty, String.Concat("TT_ELEMENT ", i, " shadow"), PlayerSocket(2))
                    End If
                Next r
            End If
        End If
        ...
        If oServerConfig.TTJackPotEnabled = True Then
            Randomize()
            Dim iRandom As Integer = Int(Rnd() * oServerConfig.TTJackpotMax)

            If iRandom = 10 Then
                For r As Integer = 0 To 8
                    If Element(r) = String.Empty Then
                        Randomize()
                        Dim p As Integer = Int(Rnd() * 8)

                        Element(p) = "jackpot"
                        Send(String.Empty, String.Concat("TT_ELEMENT ", p, " jackpot"), PlayerSocket(1))
                        Send(String.Empty, String.Concat("TT_ELEMENT ", p, " jackpot"), PlayerSocket(2))
                        Exit For
                    End If
                Next r
            End If
    ...
    Private Sub tt_placeat(ByVal incoming As String, ByVal socket As Integer)
        ...
        If IsEmpty(SquareId) = False Then
            ...
            Call blockchat(String.Empty, "Error", String.Concat(frmSystray.Winsock(socket).Tag, " placed a card on the board that already had a card in it.  Game was terminated."), PlayerSocket(1))
            Call blockchat(String.Empty, "Error", String.Concat(frmSystray.Winsock(socket).Tag, " placed a card on the board that already had a card in it.  Game was terminated."), PlayerSocket(2))

            Call objTableFunctions.EndMatch("ENDMATCH " & frmSystray.Winsock(socket).Tag, 0, False, True)
            Exit Sub
        ElseIf sCard = String.Empty Then
            Dim oGameFunctions As New GameFunctions
            oGameFunctions.KillGame(PlayerSocket(1), PlayerSocket(2), frmSystray.Winsock(socket).Tag, String.Concat("Your game was ended because ", frmSystray.Winsock(socket).Tag, " tried to place a blank card on the board."))
            Exit Sub
        End If
        ...
        If ThisPlrId = 1 Then
            Send(String.Empty, String.Concat("TT_PLACEDCARD ", SquareId, " ", PlrSquareID, " ", Replace(sCard, " ", "%20")), PlayerSocket(2))
            CardColor(SquareId) = "blue"
            Player_Hand(ThisPlrId, PlrSquareID) = String.Empty
        Else
            Send(String.Empty, String.Concat("TT_PLACEDCARD ", SquareId, " ", PlrSquareID, " ", Replace(sCard, " ", "%20")), PlayerSocket(1))
    ...
    Private Sub Analyze(ByVal iSquareID As Integer, ByVal oCard As Entities.BusinessObjects.PlayingCard, ByVal ThisPlrId As Integer)
        Try
            ...
            If IsGameOver() = False Then
                If ThisPlrId = 1 Then
                    Send(String.Empty, "TT_TURNOVER", PlayerSocket(1))
                    Send(String.Empty, "TT_YOURTURN", PlayerSocket(2))
                    System.Windows.Forms.Application.DoEvents()
                Else
                    Send(String.Empty, "TT_TURNOVER", PlayerSocket(2))
                    Send(String.Empty, "TT_YOURTURN", PlayerSocket(1))
                    System.Windows.Forms.Application.DoEvents()
                End If
            Else
                GameIsOver()
            End If
        Catch ex As Exception
            Call errorsub(ex.ToString, "Analyze")
        End Try
    End Sub
    ...
    Private Sub analyze_elemental(ByVal iSquareID As Integer, ByVal oCard As Entities.BusinessObjects.PlayingCard)
        Try
            If Element(iSquareID) = "" Then
                'EValue(iSquareID) = 0
            ElseIf oCard.Element = Element(iSquareID) Then
                EValue(iSquareID) = 1
                Send(String.Empty, String.Concat("TT_PLUSELEMENT ", iSquareID, " 1"), PlayerSocket(1))
                Send(String.Empty, String.Concat("TT_PLUSELEMENT ", iSquareID, " 1"), PlayerSocket(2))
            ElseIf oCard.Element <> Element(iSquareID) Then
                EValue(iSquareID) = -1
                Send(String.Empty, String.Concat("TT_MINUSELEMENT ", iSquareID, " -1"), PlayerSocket(1))
                Send(String.Empty, String.Concat("TT_MINUSELEMENT ", iSquareID, " -1"), PlayerSocket(2))
            End If
        Catch ex As Exception
            Call errorsub(ex.ToString, "analyze_elemental")
        End Try
    End Sub
    ...
    Private Sub analyze_checksame(ByVal oCard As Entities.BusinessObjects.PlayingCard, ByVal iSquareID As Integer, ByVal psdir1 As String, ByVal psdir2 As String, ByVal neigh1 As Integer, ByVal neigh1dir As String, ByVal neigh2 As Integer, ByVal neigh2dir As String, ByVal ThisPlrId As Integer)
        Try
            ...
            End If
        Catch ex As Exception
            Call errorsub(ex.ToString, "analyze_checksame")
        End Try
    End Sub
...
errhandler:
    ...
    Private Sub analyze_checkcombo(ByVal oCard As Entities.BusinessObjects.PlayingCard, ByVal ThisPlrId As Integer, ByVal iSquareID As Integer, ByVal psdir As String, ByVal neighbor As String, ByVal neighdir As String)
        Try
            ...
            End If
        Catch ex As Exception
            Call errorsub(ex.ToString, "analyze_checkcombo")
        End Try
    End Sub
    ...
    Private Sub analyze_checkminus(ByVal oCard As Entities.BusinessObjects.PlayingCard, ByVal iSquareID As Integer, ByVal psdir1 As String, ByVal psdir2 As String, ByVal neigh1 As Integer, ByVal neigh1dir As String, ByVal neigh2 As Integer, ByVal neigh2dir As String, ByVal ThisPlrId As Integer)
        Try
            ...
            End If
        Catch ex As Exception
            Call errorsub(ex.ToString, "analyze_checkminus")
        End Try
    End Sub
    ...
    Private Sub analyze_checkmultiply(ByVal oCard As Entities.BusinessObjects.PlayingCard, ByVal iSquareID As Integer, ByVal psdir1 As String, ByVal psdir2 As String, ByVal neigh1 As Integer, ByVal neigh1dir As String, ByVal neigh2 As Integer, ByVal neigh2dir As String, ByVal ThisPlrId As Integer)
        Try
            ...
            End If
        Catch ex As Exception
            Call errorsub(ex.ToString, "analyze_checkmultiply")
        End Try
    End Sub
    ...
    Private Sub analyze_checkplus(ByVal oCard As Entities.BusinessObjects.PlayingCard, ByVal iSquareID As Integer, ByVal psdir1 As String, ByVal psdir2 As String, ByVal neigh1 As Integer, ByVal neigh1dir As String, ByVal neigh2 As Integer, ByVal neigh2dir As String, ByVal ThisPlrId As Integer)
        Try
            ...
            End If
        Catch ex As Exception
            Call errorsub(ex.ToString, "analyze_checkplus")
        End Try
    End Sub
    ...
    Private Sub ComboAlert(ByVal iPoints As Integer)
        Try
            If (iPoints > 0) Then
                Send(String.Empty, "TT_COMBO", PlayerSocket(1))
                Send(String.Empty, "TT_COMBO", PlayerSocket(2))
            End If
        Catch ex As Exception
            Call errorsub(ex.ToString, "ComboAlert")
        End Try
    End Sub
    ...
    Private Sub MultiplyAlert(ByVal iPoints As Integer)
        Try
            If (iPoints > 0) Then
                Send(String.Empty, "TT_MULTIPLY", PlayerSocket(1))
                Send(String.Empty, "TT_MULTIPLY", PlayerSocket(2))
            End If
        Catch ex As Exception
            Call errorsub(ex.ToString, "MultiplyAlert")
        End Try
    End Sub
    ...
    Private Sub MinusAlert(ByVal iPoints As Integer)
        Try
            If (iPoints > 0) Then
                Send(String.Empty, "TT_MINUS", PlayerSocket(1))
                Send(String.Empty, "TT_MINUS", PlayerSocket(2))
            End If
        Catch ex As Exception
            Call errorsub(ex.ToString, "MinusAlert")
        End Try
    End Sub
    ...
    Private Sub SameAlert(ByVal iPoints As Integer)
        Try
            If (iPoints > 0) Then
                Send(String.Empty, "TT_SAME", PlayerSocket(1))
                Send(String.Empty, "TT_SAME", PlayerSocket(2))
            End If
        Catch ex As Exception
            Call errorsub(ex.ToString, "SameAlert")
        End Try
    End Sub
    ...
    Private Sub PlusAlert(ByVal iPoints As Integer)
        Try
            If (iPoints > 0) Then
                Send(String.Empty, "TT_PLUS", PlayerSocket(1))
                Send(String.Empty, "TT_PLUS", PlayerSocket(2))
            End If
        Catch ex As Exception
            Call errorsub(ex.ToString, "PlusAlert")
        End Try
    End Sub

    Private Sub SendTTCardData(ByVal sColor As String, ByVal x As Integer, ByVal oCard As Entities.BusinessObjects.PlayingCard)
        Send(String.Empty, String.Concat("TT_", sColor.ToUpper, "CARD ", x, " ", oCard.CardName.Replace(" ", "%20")), PlayerSocket(1))
        Send(String.Empty, String.Concat("TT_", sColor.ToUpper, "CARD ", x, " ", oCard.CardName.Replace(" ", "%20")), PlayerSocket(2))
    End Sub
    ...
    Private Sub SendTTCardFlip(ByVal iCompareID As Integer, ByVal iSquareID As Integer)
        ...
        Send(String.Empty, String.Concat("TT_CARDFLIP ", iSquareID, " ", iStatus, " ", wColor), PlayerSocket(1))
        Send(String.Empty, String.Concat("TT_CARDFLIP ", iSquareID, " ", iStatus, " " & wColor), PlayerSocket(2))
    End Sub
    ...
    Private Sub SendPoints(ByVal iPoints As Integer, ByVal ThisPlrId As Integer)
        Try
            ...
            Send(String.Empty, String.Concat("TT_POINTS ", Player1_Score, " ", Player2_Score), PlayerSocket(1))
            Send(String.Empty, String.Concat("TT_POINTS ", Player1_Score, " ", Player2_Score), PlayerSocket(2))
        Catch ex As Exception
            Call errorsub(ex.ToString, "sendpoints")
        End Try
    End Sub
...
#Region "Wall"
    Private Sub analyze_wallcombos(ByVal sType As String, ByVal iSquareID As Integer, ByVal oCard As Entities.BusinessObjects.PlayingCard, ByVal ThisPlrId As Integer)
        Try
            ...
            End Select
        Catch ex As Exception
            Call errorsub(ex.ToString, "analyze_minuswall")
        End Try
    End Sub
    ...
    Private Sub analyze_checkwall(ByVal sType As String, ByVal oCard As Entities.BusinessObjects.PlayingCard, ByVal iSquareID As Integer, ByVal psdir As String, ByVal neighbor As String, ByVal neighdir As String, ByVal backwalldir As String, ByVal ThisPlrId As Integer)
        Try
            ...
            End If
        Catch ex As Exception
            Call errorsub(ex.ToString, "analyze_checkwall")
        End Try
    End Sub
#End Region
...
#Region "Set Wall"
    Private Sub analyze_setwallcombos(ByVal sType As String, ByVal iSquareID As Integer, ByVal oCard As Entities.BusinessObjects.PlayingCard, ByVal ThisPlrId As Integer)
        Try
            ...
            End Select
        Catch ex As Exception
            Call errorsub(ex.ToString, "analyze_setwallcombos")
        End Try
    End Sub
    ...
    Private Sub analyze_checksetwall(ByVal sType As String, ByVal oCard As Entities.BusinessObjects.PlayingCard, ByVal iSquareID As Integer, ByVal psdir As String, ByVal neighbor As String, ByVal neighdir As String, ByVal backwalldir As String, ByVal ThisPlrId As Integer)
        Try
            ...
            End If
        Catch ex As Exception
            Call errorsub(ex.ToString, "analyze_checksetwall")
        End Try
    End Sub
#End Region
...
#Region "Mirror"
    Private Sub analyze_mirrorcombos(ByVal sType As String, ByVal iSquareID As Integer, ByVal oCard As Entities.BusinessObjects.PlayingCard, ByVal ThisPlrId As Integer)
        Try
            ...
            End Select
        Catch ex As Exception
            Call errorsub(ex.ToString, "analyze_checkmirror")
        End Try
    End Sub
...
#Region "Neutral"
    Private Sub analyze_neutralcombos(ByVal sType As String, ByVal iSquareID As Integer, ByVal oCard As Entities.BusinessObjects.PlayingCard, ByVal ThisPlrId As Integer)
        Try
            ...
            End Select
        Catch ex As Exception
            Call errorsub(ex.ToString, "analyze_checkneutral")
        End Try
    End Sub
...
#End Region
    ...
    Private Sub MorePower(ByVal iSquareID As Integer, ByVal ThisPlrID As Integer, ByVal oCard As Entities.BusinessObjects.PlayingCard, ByVal sDirect As String, ByVal iNeighborID As Integer, ByVal sNeighborDir As String)
        Try
            ...
            End If
        Catch ex As Exception
            Call errorsub(ex.ToString, "MorePower")
        End Try
    End Sub
    ...
    Public Function GameIsOver(Optional ByRef skipWinner As Integer = 0, Optional ByVal blnTimeOut As Boolean = False) As Boolean
        Try
            ...
            Select Case skipWinner
                ...
                Case 1
                    ...
                    If blnTimeOut = False Then
                        Surrender = True
                        Call blockchat(String.Empty, "Surrender", String.Concat(Player2, " surrendered and lost."), PlayerSocket(1))
                        Call blockchat(String.Empty, "Surrender", String.Concat(Player2, " surrendered and lost."), PlayerSocket(2))
                    End If
                Case 2
                    Player2_Score = 9
                    Player1_Score = 1
                    ...
                    If blnTimeOut = False Then
                        Surrender = True
                        Call blockchat(String.Empty, "Surrender", String.Concat(Player1, " surrendered and lost."), PlayerSocket(1))
                        Call blockchat(String.Empty, "Surrender", String.Concat(Player1, " surrendered and lost."), PlayerSocket(2))
                    End If
            End Select
            ...
            Send(String.Empty, "TT_CLOSE", PlayerSocket(1))
            Send(String.Empty, "TT_CLOSE", PlayerSocket(2))
            EndGame()

            Dim oTableFunctions As New BusinessLayer.Tables
            Call oTableFunctions.ClosePlayerTable(Tables, Player1, False)
        Catch ex As Exception
            Call errorsub(ex.ToString, "GameIsOver")
    ...
    Private Sub displaybox()
        Try
            ...
            If TradeRandomOne = True Then
                ...
                Call tt_takerandomcard(String.Concat("TT_TAKERANDOMCARD ", GameID), IIf(Winner = 1, PlayerSocket(1), PlayerSocket(2)))
                Exit Sub
            End If
            ...
            " ", Player_OriginalHand(2, 4).Replace(" ", "%20")), PlayerSocket(1))
            ...
            " ", Player_OriginalHand(1, 4).Replace(" ", "%20")), PlayerSocket(2))
        Catch ex As Exception
            Call errorsub(ex.ToString, "displaybox")
        End Try
    End Sub
    ...
    Private Sub directdisplaybox()
        Try
            ...
            Send(String.Empty, String.Concat("TT_DISPLAY ", sResult, " ", Player2, " ", iWon, " 6 ", playercards(0), " ", playercards(1), " ", playercards(2), " ", playercards(3), " ", playercards(4)), PlayerSocket(1))
            Send(String.Empty, String.Concat("TT_CARDGAIN ", playercards(0), " ", playercards(1), " ", playercards(2), " ", playercards(3), " ", playercards(4), " 1"), PlayerSocket(1))
            Send(String.Empty, String.Concat("TT_CARDLOST ", playercards(0), " ", playercards(1), " ", playercards(2), " ", playercards(3), " ", playercards(4), " 1"), PlayerSocket(2))
            ...
            Send(String.Empty, String.Concat("TT_DISPLAY ", sResult, " ", Player2, " ", iWon, " 6 ", playercards(0), " ", playercards(1), " ", playercards(2), " ", playercards(3), " ", playercards(4)), PlayerSocket(2))
            Send(String.Empty, String.Concat("TT_CARDGAIN ", playercards(0), " ", playercards(1), " ", playercards(2), " ", playercards(3), " ", playercards(4), " 1"), PlayerSocket(2))
            Send(String.Empty, String.Concat("TT_CARDLOST ", playercards(0), " ", playercards(1), " ", playercards(2), " ", playercards(3), " ", playercards(4), " 1"), PlayerSocket(1))
        Catch ex As Exception
            Call errorsub(ex.ToString, "directdisplaybox")
        End Try
    End Sub
    ...
    Private Function giftelement(ByVal winner As String) As Boolean
        Try
            For x As Integer = 0 To 8
                If Element(x) = "jackpot" Then
                    If (CardColor(x) = "blue" And PlayerID(winner) = 1) Or (CardColor(x) = "red" And PlayerID(winner) = 2) Then
                        ...
                        Call gpadd(winner, oServerConfig.TTChest, True)
                        oChatFunctions.unisend("ADDBLOCK", String.Concat("Jackpot ", winner, " %20won%20the%20jackpot%20of%20", oServerConfig.TTChest, "GP.", " ", CStr(System.Drawing.ColorTranslator.ToOle(System.Drawing.Color.Yellow))))
                        Call resetjackpot()
                        Return True
                    End If
                End If
            ...
            Return False
        Catch ex As Exception
            Call errorsub(ex.ToString, "giftelement")
        End Try
    End Function
    ...
    Private Sub clearboard()
        Try
            ...
            For x As Integer = 0 To 8
                ...
                If Elemental = True Then
                    Send(String.Empty, String.Concat("TT_ELEMENT ", x, " ", Element(x)), PlayerSocket(1))
                    Send(String.Empty, String.Concat("TT_ELEMENT ", x, " ", Element(x)), PlayerSocket(2))
                End If
            Next
            ...
            UpdateHand()
        Catch ex As Exception
            Call errorsub(ex.ToString, "clearboard")
        End Try
    End Sub
    ...
    Private Sub SendCards()
        Try
            For x As Integer = 0 To 4
                Send(String.Empty, String.Concat("TT_HAND ", x, " ", Player_Hand(1, x).Replace(" ", "%20")), PlayerSocket(1))
                Send(String.Empty, String.Concat("TT_HAND ", x, " ", Player_Hand(2, x).Replace(" ", "%20")), PlayerSocket(2))
            Next
            ...
            Send(String.Empty, String.Concat("TT_STARTER ", iStarter), PlayerSocket(1))
            Send(String.Empty, String.Concat("TT_STARTER ", iStarter), PlayerSocket(2))
        Catch ex As Exception
            Call errorsub(ex.ToString, "SendCards")
        End Try
    End Sub
    ...
    Private Sub sDeath()
        ...
        'Send(String.Empty, "TT_CLEARBOARD", PlayerSocket(1))
        'Send(String.Empty, "TT_CLEARBOARD", PlayerSocket(2))
        ...
        Send(String.Empty, String.Concat("TT_PICKCARDS 0"), PlayerSocket(1))
        Send(String.Empty, String.Concat("TT_PICKCARDS 0"), PlayerSocket(2))
